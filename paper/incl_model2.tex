...
On the other hand, $\homogdiss(\alpha)$ is a non-negative function that is zero ONLY IF $\alpha=0$ and monotonically increases with $\alpha$, reaching $\homogdiss(1)=1$. 
...
% 
The displacement field of the substrate WHICH IS elastically compatible to the homogeneous strain, is the linear function $v(x, t) = \bar\epsilon/2 (2x-1) t$, where $\bar\epsilon$ is the applied tensile strain rate and $t$ plays the role of a loading time parameter. 
% 
...
With the assumption that the loading process is slow enough for inertia and dynamic effects to be negligible, this evolutionary model leads to identyfing a sequence of equilibrium configurations as critical states, to determinate the stability of these configurations, and to  IDENTIFY the trajectories through which the system transitions between different equilibrium states.
% 
...
Therefore, the solution to~\eqref{auto1} for states such that $u''(x)=\alpha'(x)= 0, \, \forall x\in (0, 1)$ is the homogeneous branch
...
by our choice to model the material through a quadratic dissipation term $\homogdiss(\alpha)$, IN OUR softening MODEL damage necessarily starts as soon as $t>0$.}
...
{We} extract information on the onset of instability  seeking a solution in Fourier series of the fields $v$ and $w$ in \eqref{hessian22}, such that $v(x)=\sum_{n=1}^{\infty} a_{n} \sin \left(n \pi x+\phi_{n}\right), \quad w(x)=\sum_{n=1}^{\infty} b_{n} \cos \left(n \pi x+\psi_{n}\right)$. Then, we observe that, thanks to boundary conditions, $\psi_{n}=\phi_{n}=0$ for all natural $n$. The stability condition~\eqref{eqn:linear_second_order_stability}  takes the form:
...
Conversely, the stability problem~\eqref{eq:variational_stability}, a constrained eigenvalue problem in a convex cone, is OF A DIFFERENT NATURE ALTOGETHER DUE TO THE TOPOLOGICAL DIFFERENCE OF THE UNDERLYING ENERGY SPACE. THE ASSOCIATED DISCRETE PROBLEM CAN BE numerically solved by exploiting the orthogonality between the set $\conespace$ and its dual $K^*:=\{y \in H^1(0,1): \langle x,y \rangle \leq 0, \forall x \in \conespace\}$, cf.~\cite{moreau, pinto-da-costa}.
