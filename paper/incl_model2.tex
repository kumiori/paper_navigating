...
\end{equation}
where $\homogdiss(\alpha)$ can be interpreted as the energy dissipated during an homogeneous damaging process, COMPUNDS A term that is proportional to the square of its gradient and controls the energy cost of spatial damage variations. In the first summand, $\soften(\alpha)$ is the function that describes the material softening. %
For physical consistency, $\soften(\alpha)$ is a non-negative function that is one when $\alpha=0$ and monotonically decreases as $\alpha\to 1$, reaching zero for $\alpha=1$ The damage-dependent stress IS $\sigma(\alpha):=\mathsf{E}a(\alpha) e$. 
On the other hand, $\homogdiss(\alpha)$ is a non-negative function that is zero ONLY IF $\alpha=0$ and monotonically increases with $\alpha$, reaching $\homogdiss(1)=1$. The parameter $\damagell$ is a characteristic length that controls the competition between localisation and homogeneous damage, effectively controlling the width of damage localisations. The (positive) parameter $\ell$ is a characteristic length of the underlying material that controls the peak stress of the material in one-dimensional traction experiments.
...
% 
The displacement field of the substrate WHICH IS elastically compatible to its strain, is the linear function $v(x, t) = \bar\epsilon/2 (2x-1) t$, where $\bar\epsilon$ is the applied tensile strain rate and $t$ plays the role of a loading time parameter. 
% 
The structure's reference configuration is the interval $(0, L)$. The film is subjected to a DISTRIBUTED ELASTICALLY  compatible  tensile imposed LOAD $v(x, t)$ AND displacements at its free ends $x = 0, L$, so that for all $t$, $u(0)=v(0, t)$ and $u(L)=v(L, t)$
% 
...
\begin{equation}
    ...
    \Lambda := \frac{\mathsf{E_{\text{2d}}}}{\mathsf{E'_{\text{2d}}}}\frac{h W_1}{L^2}, \quad
    \stiffratio := \frac{\mathsf{E_{\text{2d}}}}{\mathsf{E'_{\text{2d}}}}\frac{W_1}{h}, \quad
\end{equation}
while spatial variables and physical displacements are respectively normalised with respect to the film's length $L$ and the displacement scale $u_0 := \frac{\mathsf{w_1}L}{\mathsf{E_{\text{2d}} h}}$, WHICH FIXES SCALE OF THE NON-DIMENSIONAL DISSIPATED ENERGY.
...
intuitive idea that a state is observable (if and) only if it is stable, and in turn, a state is stable only if it is a local minimum of the energy among admissible state perturbations, FOR EACH GIVEN LOADING CONDITION. 
...
With the assumption that the loading process is slow enough for inertia and dynamic effects to be negligible, this evolutionary model not only leads to identyfing a sequence of equilibrium configurations as critical states, also to the determination of the stability of these configurations, and to THE IDENTIFICATION OF the trajectories through which the system transitions between different equilibrium branches.
% 
...
As a consequence, irreversibility restricts the admissible set of competitors to the set $K^+_{\alpha_t}:=H^1(0, 1) \times \{\beta\in H^1(0, 1): \beta\geq \alpha_t\}$ (respectively, $\widetilde{K^+_{\alpha_t}}:=K^+_{\alpha_t}\times H^1(0, 1)$), where $\alpha_t$ is the damage field at time $t$, IN TURN, UNKNOWN AT TIME $t$. In the irreversible case the set of admissible competitor states depends explicitly on the entire history of the evolution through the current damage field.
...
In the subsequent section, we exploit the energy-stability statement~\eqref{} for both the mechanical models models introduced above, exploring the equilibrium configurations and identifying the conditions under which the system transitions between different equilibrium branches. 
...
The first order variation of the energy functional $\Estiff$ \added[id=ALB]{in the direction $z:=(v, w)$} is given \added[id=ALB]{by the following linear form}  
\begin{equation}
...
\added[id=ALB]{
    This differential system is equipped with essential kinematic boundary conditions $u(0) = \subsu(0, t), u(1) = \subsu(1, t)$, whereas conditions on the internal field $\alpha'(0)=\alpha'(1)=0$ naturally follow by minimality.
...
Notice that the choice of boundary conditions for displacements compatible with the substrate's deformation implies that the pair $\yhom:=(u_h(x), \alpha_h)$ given by $u_h(x)\equiv \subsu(x, t)$\comment[id=ALB]{notation $
\bar u_t$} and $\alpha_h$ a load-dependent constant to identify, is always (the unique homogenous) solution to the first order equilibrium equations. This makes it immediate to identify the fundamental homogeneous solution branch $t \mapsto \yhom$ and to decouple the elasticity problem from the  evolution of damage. 
%  driving stress softening.
}
Therefore, solving~\eqref{auto1} for states such that $u''(x)=\alpha'(x)= 0, \, \forall x\in (0, 1)$ identifies the components of the homogeneous solution
...
\added[id=ALB]{
Notice that,  
THE CRITICAL DAMAGE LOAD COMPUTED IS A CONSEQUENCE OF FIRST ORDER CONDITIONS, NAMELY.
by our choice to model the material through a quadratic term dissipation term $\homogdiss(\alpha)$, IN OUR MODELS
damage necessarily starts as soon as $t>0$.}
...
\end{equation}
% \comment[id=ALB]{CHECK, looks like only homogeneous damage energy}


% ---------------------------

% \comment[id=ALB]{How to understand linear/nonlinear}
An equilibrium configuration $y_t:=(u,\alpha)_t$ is a state such that the first variation $\delta \Estiff(y_t)(z)$ vanishes for all admissible test fields \added[id=ALB]{in the vector space $X_0$}. 
To assess the incremental stability of the homogeneous solution in the reversible (linear) case, we examine the \added[id=ALB]{positivity of the} second variation, requiring
\begin{equation}
\delta^2 \Estiff(y_t)(y_t-z, y_t-z)>0, \qquad  \forall z\in X_0,
\label{eqn:linear_second_order_stability}
\end{equation} \comment[id=ALB]{check }
...
\label{auto2}
\end{eqnarray}
It is easy to show that the homogeneous solution on the trivial branch remains the same as in the case of the rigid substrate, that is $\alpha_h = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2}\label{eq:homo11}$, whereas the effective elastic energy along the homogeneous branch now reads $\Ecompl_\homogdiss(\bar{\epsilon}) = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2} + \frac{r_s}{2}\bar{\epsilon}^2$.\comment[id=ALB]{why there is no damage part?, CHECK} 

We once more seek the linear incremental stability of an equilibrium configuration $y_t := (u, \alpha,  v)_t$ satisfying that the first order condition $\delta \Ecompl(y_t)(y-y_t)=0$ \added[id=ALB]{for all admissible state perturbations $z:=y-y_t\in X_0$} by examining the positivity of the second variation, namely
\begin{equation*}
...
The former allows to compute the elastic equilibrium displacment $u_t$ which can thus be eliminated from energy\comment[id=ALB]{miracle in 1d, expand}, while the latter governs the evolution of the damage field.
...
\end{equation}
where $e$ is the elastic strain and $\soften'(\alpha)$ and $\homogdiss'(\alpha)$ are the derivatives of the softening and dissipation energy densities with respect to the damage variable.
...
$\mathcal{R}(\alpha):=\{e: \mathsf{E}e^2 \leq -\frac{2 \mathsf{w}'(\alpha)}{\mathsf{a}'(\alpha)}\},$ and $
\mathcal{R}^*(\alpha):=\{\sigma: \frac{\sigma^2}{\mathsf{E}} \leq \frac{2 \mathsf{w}'(\alpha)}{\mathsf{s}'(\alpha)}\}$, respectively.
% 
% 
% 
...
To this end, we can differentiate~\eqref{eq:eq_variational_inequality_full} with respect to $t$ to obtain a boundary value problem relating the rate of evolution $\dot y_t$ to the current state $y_t$, supposing the latter known, namely
% 
\begin{equation}
...
\begin{equation}
    ...
    \delta^2 \Psi(y_h)(\zeta, \zeta) > 0, \quad \forall \zeta \in X_0,
\end{equation}
...
Denoting $t_s$ the load at which the homogeneous solution looses stability by analogy to the bifurcation load, namely $t_s: = \inf_t \{\delta^2 \Psi(y_t)(\zeta, \zeta) =0, \forall \zeta \in K^+_0 \}$, the set inclusion $K^+_0 \subset X_0$ implies that necessarily $t_b \leq t_s$. (EQUALITY OCCURS WHEN THE FIRST BIFURCATION MODE IS NONNEGATIVE)
}
\added[id=ALB]{
...
A sufficient condition for the stability of the homogeneous state $y_h$ is given by the strict positivity  of the Hessian form, on the constrained space of admissible perturbations, namely (for the stiff substrate model)
}
...
In practice, the stability problem~\eqref{eq:variational_stability}, a constrained eigenvalue problem in a convex cone, is OF A DIFFERENT NATURE ALTOGETHER DUE TO THE TOPOLOGICAL DIFFERENCE OF THE UNDERLYING ENERGY SPACE. THE ASSOCIATED DISCRETE PROBLEM CAN BE numerically solved by exploiting the orthogonality between the set $\conespace$ and its dual $K^*:=\{y \in H^1(0,1): \langle x,y \rangle \leq 0, \forall x \in \conespace\}$, cf.~\cite{moreau, pinto-da-costa}.
