...
\definechangesauthor[color=orange]{ALB}
...
\affil[3]{Sorbonne Université, CNRS, Institut Jean Le Rond d'Alembert, F-75005 Paris, France}
...
\newcommand{\Youngsubs}{{\mathsf{E_{\text{2d}}'}}}
...
\begin{table}[h!]
    \centering
    \begin{tabular}{  m{6.5cm}  m{3cm}  m{5cm}  }
        \hline
        ...
        Internal damage length              & $\bar\damagell$                                    & dimensional, [m]                                                                \\
        ...
        State, perturbations (stiff)        & $(u,\alpha), (\utest, \beta)$                      &                                                                                 \\
        State, perturbations (compliant)    & $(u, \alpha, \subsu), (\utest, \beta, \subsutest)$ &                                                                                 \\
        ...
        Eigenvalue at load $t$              & $\lambda_t$                                        &                                                                                 \\
        %
        FEM state, generic                  & $\femstate$                                        &                                                                                 \\
...
In both cases, one deals with the problem of finding  configurations that satisfy
$\min_{{u}} \Psi({u}), \min_{{u},\alpha} \Psi({u},\alpha)$
or at least \emph{some} necessary conditions for energy optimality and, potentially, accounting for constraints.
In the nonconvex setting, multiple minima of the functionals and the multitude of equilibrium states accessible during loading spawn many possible evolution paths. One can expect that under a quasi-static loading protocol, the system navigates among metastable states which are continuous branches of equilibria. These branches can bifurcate and intersect as well as terminate at points where the state stability is lost.
%
...
During an isolated switching event, new equilibrium branches can be determined using a steepest descent or a continuation algorithm. The path selection may suffer from indeterminacy, however, because the energy functionals in our focus are strongly nonlinear, they lack convexity in their argument and thus exhibit multiple local minima, or none at all.
Consequently, conducting stability and bifurcation analyses becomes crucial to distinguish among the various potential solutions or evolution paths, those that are physically relevant.
...
For systems of ODEs the criterion  of bifurcation (from a fundamental solution) amounts to the study of the existence of solutions different than the fundamental one in an arbitrary neighborhood of the control parameters. Conditions of failure of the implicit function theorem~\cite{Iooss2012-el} describe the scenario under which a system of equations can realize more than one smooth solution.
...
An \emph{equilibrium map} can be constructed in this setup, allowing all  {inhomogeneous} solutions connected to the homogeneous branch to be identified along with their stability.
This enables us to monitor the solutions returned by various numerical optimization techniques and assess their observability.
Our findings indicate that under quasi-static loading conditions, line-search-based descent algorithms not relying on full Hessian can fail to detect expected branch-switching events and may return solutions that persist on unstable branches, thus lacking physical relevance.
...
We use the prime sign to indicate spatial derivatives.
%
%
...
The structure under consideration is a multilayer composite constituted by a brittle thin film  made of the material identified by the state function $W$, attached to an underlying substrate which is either {stiff} or elastically compliant. The thin film is a one-dimensional membrane with thickness $h$ and length $L$ with $L\gg h$, subject to a combination of imposed displacements by the substrate and loadings at the boundary. The structure's reference configuration is the interval $(0, L)$.
%
The substrate is modelled as a one-dimensional elastic foundation whose displacement field is $v(x)$.
%
The displacement field which is elastically compatible to an homogeneous strain in the substrate is the linear function $v(x, t) = \bar\epsilon_t/2 (2x-1)$, where {$\bar\epsilon_t\in \mathbb R$} is the {average} strain and $t>0$ plays the role of a loading time parameter.
%
%
The film is subjected to {the displacement of the substrate} $v(x, t)$  and to given (compatible) displacements at its free ends $x \in \{0, L\}$, so that for all $t$, $u(0)=v(0, t)$ and $u(L)=v(L, t)$
%
Our first model describes a brittle thin film deposited on a stiff, non-deformable substrate. This model assumes the substrate is {stiff}, meaning, $\subsu$ is a given. The elastic interaction is modelled by a distributed linear elastic foundation of stiffness $\mathsf{K}$, thus
%
...
\begin{equation}
    ...
    \Psi( u, \alpha) = \int_{0}^{1} \left[ \frac{1}{2} \ \soften(\alpha)(u')^2
        ...
        \right] dx,
\end{equation}
where $\Lambda^2 = \frac{\mathsf E_{\text{eff}}}{\mathsf{K}}$, $E_{\text{eff}}$ being the effective stiffness of the two dimensional {membrane} {and spatial variables as well as physical displacements are respectively normalized with respect to the film's length $L$ and the displacement scale $u_0 := \frac{\mathsf{w_1}L}{\mathsf{K}}$}.
...
\begin{equation}
    ...
    %
    \Lambda := \frac{\Youngfilm}{\Youngsubs}{\frac{\mathsf h \mathsf h'}{L^2}}, \quad
    %
    \stiffratio := \frac{\Youngsubs}{\Youngfilm}{\frac{\mathsf{h'}}{\mathsf h}}. \quad
\end{equation}
...
we seek {stable states exploiting a notion of directional stability.}
    {The system at $y$ is directionally stable in the admissible direction $z$ if:}
\begin{equation}
...
Admissible perturbations in the fully reversible case belong to the {tangent} space {$T_y X_t=X_0$} associated with  $X_t$ (respectively, $\widetilde{X_t}$) {which is a linear vector space}, whereas in the irreversible case {admissible perturbations constitute} the closed convex cone $K^+_0$ (respectively, $\widetilde{K^+_0}$).
...
\section{Linear and nonlinear stability {of homogeneous solutions}}
\label{sec:stability}
...
\end{equation}
%
By using standard arguments of the calculus of variations, localizing the integral and choosing $\beta = 0$ first, and then $v =0$ leads to establishing the strong form of local stationarity conditions. The mechanical equilibrium, the damage criterion, {and the associated boundary conditions, } are thus respectively given by
\begin{eqnarray}
    %
    \begin{cases}
...
\end{equation}
Notice that,
first order conditions identify the critical load threshold that activates the damaging process. The quantity $\bar\epsilon_*^c$, the value of the load for which the undamaged elastic state becomes unstable, is determined injecting the elastic solution $u_h$ in the energy and computing $\delta \Estiff(u_h, 0)(0, \beta)=0$.
...
\begin{equation}
    \delta^2 \Estiff(y_t)(y-y_t, y-y_t)>0, \qquad  \forall {y-y_t}\in X_0,
...
Then we observe that, thanks to boundary conditions, $\psi_{n}=\phi_{n}=0$ for all natural $n$. The stability condition~\eqref{eqn:linear_second_order_stability} {for homogeneous states $\yhom$} takes the form:
...
\begin{figure}
    \centering
    \begin{overpic}[width=\linewidth]{hessian-models.pdf}
        %
...
Arguing similarly to the stiff case by localization and integration by parts, for all $x \in (0, 1)$ the Euler-Lagrange equations and associated boundary conditions read
\begin{eqnarray}
    \label{modeld_el_1}
    \begin{cases}
...
Similar to the previous section we claim that, whenever the first order term vanishes, the system is stable only if $\delta^2 \Ecompl
    >0$ for all sufficiently smooth admissible test fields $(\utest, \beta, \subsutest)$ in the vector space $\widetilde{ X_0}$. The stability condition {for the homogeneous state $\yhom$}  takes the form
\begin{align}
    %
    %
...
\end{equation}
Here, $\phi_t$ is the scalar function associated with  the variation of elastic energy density {computed} at the equilibrium {and} defined by $\delta \Estiff(y_t)(0, \beta) = \langle -\phi_t(\alpha_t), \beta\rangle$.
...
\begin{figure}
    \centering
    \hspace*{-.3cm}
    \includegraphics[align=c, width=.6\textwidth]{model_stiff_energy.pdf}
    \includegraphics[align=c, width=.4\textwidth]{model_stiff_fields.pdf}
    %
    \includegraphics[align=c,width=.8\textwidth]{bifurcations_model1_annotated.pdf}
...
\begin{figure}
    \centering
    \hspace*{-.3cm}
    \includegraphics[align=c,width=.7\textwidth]{model_compliant_energy.pdf}
    \includegraphics[align=c,width=.3\textwidth]{model_compliant_fields.pdf}
    \includegraphics[align=c,width=.8\textwidth]{bifurcations_model2_annotated.pdf}
...
Because eigenvalues represent the local curvatures of the energy functional, the sign of the smallest  $\lambda_t$ determines the stability of the solution, with {${\inf}\lambda_t > 0$} indicating stability and $\inf\lambda_t < 0$ indicating instability.
...
\begin{figure}
    ...
    \hspace*{-.3cm}
    %
    \includegraphics[width=.6\textwidth]{model_stiff_energy_kick.pdf}
    \includegraphics[width=.4\textwidth]{model_stiff_profiles.pdf}
...
\begin{figure}
    \centering
    \hspace*{-.3cm}
    \includegraphics[width=.6\textwidth]{model_compliant_energy_kick.pdf}
    \includegraphics[width=.4\textwidth]{model_compliant_profiles.pdf}
    %
    %
...
\begin{figure}[htbp]
    \centering
    \includegraphics*[align=c,width=.45\textwidth]{model_stiff_kick_profiles-1.pdf}
    \includegraphics*[align=c,width=.45\textwidth]{model_compliant_kick_profiles-1.pdf}
    %
    %
...
\begin{figure}
    %
    \hspace*{-.3cm}
    \includegraphics[width=.5\textwidth]{model_stiff_energy_kick_algo.pdf}
    \includegraphics[width=.5\textwidth]{model_compliant_energy_kick_algo.pdf}
    %
    %
    %
    %
...
The algorithm is repeated until convergence is achieved {on $x^{(k)}$}. Note that, in the cone-constrained case, the residual vector $y^{(k)}$ need not be zero at convergence.
...

    {
        ...
        Following loading, the numerical solver switches from the homogeneous solution to a localised pattern. The corresponding trajectory is shown in the equilibrium map in Figure~\ref{fig:branches-stiff} with a thick black solid line. The irreversible homogeneous solution is displayed with black dashed line.
        %
...
\begin{figure}[htbp]
    \centering
    \includegraphics*[width=.95\textwidth]{profiles-bif-stab-7f4361886184f3c6791fe16bf4f4b3f2.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics*[width=.47\textwidth]{spectra_irrev_annotated.pdf}
    \includegraphics*[width=.47\textwidth]{irreversibility_alpha_evo_annotated.pdf}
    %
    %
    %
    %
...
\begin{thebibliography}{10}
    ...
    \newblock Technical Report ANL-95/11 - Revision 3.15, Argonne National Laboratory, 2021.
    ...
    \newblock In E.~Arge, A.~M. Bruaset, and H.~P. Langtangen, editors, {\em Modern Software Tools in Scientific Computing}, pages 163--202. Birkh{\"{a}}user Press, 1997.

    \bibitem{Baldelli2021-gc}
    ...
    \newblock {\em J. Mech. Phys. Solids}, 152(104424), 2021.

    \bibitem{Bazant2010-zb}
    Z~Ba{\v z}ant and L~Cedolin.
    \newblock Stability of structures: Elastic, inelastic, fracture and damage theories.
    \newblock {\em Journal of Structural Engineering-asce}, 119:1001--1002, August 2010.
    ...
    \newblock {\em Phys. Rev. B Condens. Matter}, 78(18):184107, November 2008.

    \bibitem{Bettiol2020-ey}
    R~G Bettiol and P~Piccione.
    \newblock Instability and bifurcation.
    \newblock {\em Not. Am. Math. Soc.}, 2020.

    \bibitem{bharali:2022}
    Ritukesh Bharali, Somdatta Goswami, Cosmin Anitescu, and Timon Rabczuk.
    \newblock A robust monolithic solver for phase-field fracture integrated with fracture energy based arc-length method and under-relaxation.
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 394:114927, May 2022.

    \bibitem{Bochkanov2013-lk}
    S~Bochkanov and V~Bystritsky.
    \newblock Alglib.
    \newblock {\em Available from: www alglib net}, 2013.

    \bibitem{Bosco:2020aa}
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 349:167--190, June 2019.

    \bibitem{Clayton2011-xq}
    J~D Clayton and J~Knap.
    \newblock A phase field model of deformation twinning: Nonlinear theory and numerical simulations.
    \newblock {\em Physica D}, 240(9):841--858, April 2011.

    \bibitem{Combescure2016-dy}
    ...
    \newblock {\em Int. J. Solids Struct.}, 88-89:296--318, June 2016.

    \bibitem{Conti2004-yj}
    ...
    \newblock {\em Arch. Ration. Mech. Anal.}, 173(1):69--88, July 2004.

    \bibitem{Curtis2015-wp}
    ...
    \newblock {\em Math. Program. Comput.}, 7(4):399--428, December 2015.

    \bibitem{Dai1999-hz}
    Y~H Dai and Y~Yuan.
    \newblock A nonlinear conjugate gradient method with a strong global convergence property.
    \newblock {\em SIAM J. Optim.}, 10(1):177--182, January 1999.

    \bibitem{De-Lorenzis2020-rz}
    ...
    \bibitem{Doedel1981-sa}
    Eusebius~J Doedel.
    \newblock {AUTO}: A program for the automatic bifurcation analysis of autonomous systems.
    \newblock {\em Congr. Numer.}, 30(265-284):25--93, 1981.

    \bibitem{Faurie2019-to}
    ...
    \newblock {\em Acta Mater.}, 165:177--182, February 2019.

    \bibitem{Finel2010-zw}
    ...
    \newblock {\em C. R. Phys.}, 11(3):245--256, April 2010.

    \bibitem{fuster-lopez:2020-picassos}
    ...
    \newblock {\em SN Applied Sciences}, 2(12):2159, 2020.

    \bibitem{Godard2022-ss}
    ...
    \newblock {\em Scr. Mater.}, 213:114638, May 2022.

    \bibitem{Goehring2010-xz}
    ...
    \newblock {\em Soft Matter}, 6(15):3562--3567, July 2010.

    \bibitem{Grimstad2016-cq}
    ...
    \newblock {\em J. Global Optimiz.}, 65(3):401--439, July 2016.

    \bibitem{Guenole2020-tc}
    ...
    \newblock {\em Comput. Mater. Sci.}, 175:109584, April 2020.

    \bibitem{Harandi2023-cd}
    ...
    \newblock {\em Int. J. Mech. Sci.}, 258:108568, November 2023.

    \bibitem{Hill1958-xd}
    R~Hill.
    \newblock A general theory of uniqueness and stability in elastic-plastic solids.
    \newblock {\em J. Mech. Phys. Solids}, 6(3):236--249, May 1958.
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 368:113106, 2020.
    ...
    \newblock {\em Int. J. Plast.}, 143:103019, August 2021.

    \bibitem{Hutchinson2013-jk}
    John~W Hutchinson.
    \newblock The role of nonlinear substrate elasticity in the wrinkling of thin films.
    \newblock {\em Philos. Trans. A Math. Phys. Eng. Sci.}, 371(1993):20120422, June 2013.

    \bibitem{Iooss2012-el}
    Gerard Iooss and Daniel~D Joseph.
    \newblock {\em Elementary stability and bifurcation theory}.
    ...
    \newblock {\em Int. J. Solids Struct.}, 82:95--110, March 2016.

    \bibitem{Kirkesaether_Brun2020-wa}
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 361:112752, April 2020.

    \bibitem{Kristensen2020-zy}
    ...
    \newblock {\em Theor. Appl. Fract. Mech.}, 107:102446, June 2020.

    \bibitem{Kuhn2015-rt}
    Charlotte Kuhn, Alexander Schl{\"u}ter, and Ralf M{\"u}ller.
    \newblock On degradation functions in phase field fracture models.
    \newblock {\em Comput. Mater. Sci.}, 108:374--384, October 2015.

    \bibitem{Kumar2020-xz}
    A~Kumar, B~Bourdin, G~A Francfort, and {others}.
    \newblock Revisiting nucleation in the phase-field approach to brittle fracture.
    \newblock {\em J. Mech. Phys. Solids}, 142(104027), 2020.

    \bibitem{Baldelli2014-ho}
    Andres~A. Leon~Baldelli, J-F Babadjian, Blaise Bourdin, Duvan Henao, and Corrado Maurini.
    \newblock A variational model for fracture and debonding of thin films under in-plane loadings.
    \newblock {\em Journal of the Mechanics and Physics of Solids}, 70:320--348, 2014.

    \bibitem{Leon_Baldelli2015-rp}
    ...
    \newblock {\em J. Elast.}, 121(2):275--301, December 2015.

    \bibitem{Lewis2013-eu}
    Adrian~S Lewis and Michael~L Overton.
    \newblock Nonsmooth optimization via quasi-newton methods.
    \newblock {\em Math. Program.}, 141(1):135--163, October 2013.

    \bibitem{Liu1989-kl}
    Dong~C Liu and Jorge Nocedal.
    \newblock On the limited memory {BFGS} method for large scale optimization.
    \newblock {\em Math. Program.}, 45(1-3):503--528, August 1989.

    \bibitem{Liu2013-sj}
    G~R Liu and S~S Quek.
    \newblock {\em The Finite Element Method: A Practical Course}.
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 399:115410, September 2022.

    \bibitem{Lookman2003-gd}
    T~Lookman, S~R Shenoy, K~{\O} Rasmussen, A~Saxena, and A~R Bishop.
    \newblock Ferroelastic dynamics and strain compatibility.
    \newblock {\em Phys. Rev. B: Condens. Matter Mater. Phys.}, 67(2):024114, January 2003.

    \bibitem{francfort_marigo1998}
    J.-J. Marigo and G.~A. Francfort.
    \newblock Revisiting brittle fracture as an energy minimization problem.
    \newblock {\em J. Mech. Phys. Sol.}, 35(7):1319--1342, 1998.

    \bibitem{Matthies1979-gl}
    Hermann Matthies and Gilbert Strang.
    \newblock The solution of nonlinear finite element equations.
    \newblock {\em Int. J. Numer. Methods Eng.}, 14(11):1613--1626, January 1979.

    \bibitem{Mayer2022-km}
    ...
    \newblock {\em Comput. Mater. Sci.}, 206:111266, April 2022.

    \bibitem{Miehe2010-sj}
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 199(45):2765--2778, November 2010.

    \bibitem{Millan:2023aa}
    ...
    \newblock {\em Nature Communications}, 14(1):6914, 2023.

    \bibitem{Moreau1962-fz}
    J~J Moreau.
    ...
    \bibitem{najmeddine:2024}
    Aimane Najmeddine and Maryam Shakiba.
    \newblock Efficient {BFGS} quasi-newton method for large deformation phase-field modeling of fracture in hyperelastic materials.
    \newblock {\em Eng. Fract. Mech.}, 310(110463):110463, November 2024.

    \bibitem{Nocedal2006-qh}
    J~Nocedal and S~J Wright.
    \newblock {Quasi-Newton} methods.
    ...
    \newblock {\em Universit{\'e} Pierre er Marie-Curie}, Ph.D. thesis, 2009.

    \bibitem{Salman2021-mn}
    O~U Salman and L~Truskinovsky.
    \newblock De-localizing brittle fracture.
    \newblock {\em J. Mech. Phys. Solids}, 154(104517):104517, September 2021.

    \bibitem{Salman2019-kp}
    O~U Salman, G~Vitale, and L~Truskinovsky.
    \newblock Continuum theory of bending-to-stretching transition.
    \newblock {\em Phys Rev E}, 100(5-1):051001, November 2019.
    ...
    \newblock {\em Eur. Phys. J. B}, 92(1):20, January 2019.

    \bibitem{SALMAN2012219}
    O.U. Salman and L.~Truskinovsky.
    \newblock On the critical nature of plastic flow: One and two dimensional models.
    \newblock {\em International Journal of Engineering Science}, 59:219--254, 2012.

    \bibitem{Sanderson2016-ht}
    Conrad Sanderson and Ryan Curtin.
    \newblock Armadillo: a template-based c++ library for linear algebra.
    \newblock {\em J. Open Source Softw.}, 1(2):26, June 2016.
    ...
    \newblock {\em Philosophical Magazine Letters}, 100(6):294--305, 06 2020.

    \bibitem{Simone2012-tx}
    ...
    \newblock {\em MATHEMATICS NOTEBOOKS}, 27, 2012.

    \bibitem{Stiefel1952-fw}
    Eduard Stiefel.
    \newblock Methods of conjugate gradients for solving linear systems.
    \newblock {\em J. Res. Natl. Bur. Stand.}, 49:409--435, 1952.

    \bibitem{Storvik2021-cd}
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 381:113822, August 2021.
    ...
    \newblock {\em Adv. Mater.}, May 2024.

    \bibitem{Truskinovsky2010-st}
    L~Truskinovsky.
    ...
    \newblock {\em Received August}, 2010.
    ...
    \bibitem{weiss:2017-linking}
    J{\'e}r{\^o}me Weiss and V{\'e}ronique Dansereau.
    \newblock Linking scales in sea ice mechanics.
    \newblock {\em Philos Trans A Math Phys Eng Sci}, 375(2086), Feb 2017.

    \bibitem{Weiss2021-db}
    ...
    \newblock {\em Comput. Methods Appl. Mech. Eng.}, 360:112704, 2020.
    ...
    \newblock {\em Ann. Oper. Res.}, 103(1):213--234, March 2001.
    ...
    \newblock {\em Eur. Phys. J. E Soft Matter}, 43(9):56, September 2020.
    ...
    \newblock {\em Phys Rev E}, 102(2-1):023006, August 2020.

    \bibitem{zolesi:2024}
    Camilla Zolesi and Corrado Maurini.
    \newblock Stability and crack nucleation in variational phase-field models of fracture: Effects of length-scales and stress multi-axiality.
    \newblock {\em Journal of the Mechanics and Physics of Solids}, 192:105802, 2024.
