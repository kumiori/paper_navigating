...
It uses collocation with Lagrange polynomials to discretize the boundary-value problem, in our simulations we had $300$ mesh points with $5$ collocation nodes and activated mesh adaptation.
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.6\textwidth}
        \centering
        \includegraphics[align=c,width=\linewidth]{../images/model_compliant_energy.pdf}
        \caption{First stacked figure}
        \vspace{5pt} % Space between the two stacked figures
        \includegraphics[align=c,width=\linewidth]{../images/bifurcations_model2_annotated.pdf}
        \caption{Second stacked figure}
    \end{minipage}
    \begin{minipage}{0.35\textwidth}
        \centering
        \includegraphics[align=c,width=\linewidth]{../images/model_compliant_fields.pdf}
        \caption{Tall figure in second column}
    \end{minipage}
\end{figure}
...
Firstly, it rules out transitions between branches that require a local decrease of damage in favour of a global decrease of energy (e.g. between $n=3$ to $n=4$, see profiles in the Fig.~\ref{fig:kick}(left) of the transition (a)$\mapsto$(b)).
...
The following example illustrates the subtle scenario in which two stable irreversible solutions are computed, one of which branches off the homoogeneous solution at a bifurcation point without any instability.
Despite allowing potential bifurcations, the second computation shows that the system can robustly navigate a purely homogeneous branch, satisfying a sufficient condition for stability (hence for observability) of the computed trajectory.
...
\albadd{
    The numerical solution of first order optimality conditions with irreversibility constraints is tackled leveraging the minbleic subpackage of the Alglib library~\cite{Bochkanov2013-lk}, via an active set method that handles inequality constraints in the first order equilibrium (or criticality) problem, encoded by inequality~\eqref{eq:eq_variational_inequality_full}.
    % 
...
More in particular, we implement a simple iterative Scaling-and-Projection algorithm~\cite{Pinto_da_Costa2010-qv} which depends upon one numerical parameter, a scaling factor $\eta>0$. Given a convex cone  $K$ and an initial guess $z_0$ (not necessarily in the convex cone), the algorithm operates by first projecting the vector in the cone, $x^{(k=0)}= \operatorname{P_K}(z_0)$, then, the iterative estimate for the eigenvalue can be computed using the Rayleigh quotient
\begin{equation}
...
using two different numerical methods: the L-BFGS/CG method (via ALGLIB) and a second solver based on PETSc's reduced-space (active set) algorithms for variational inequalities (via SNES).
...
\albadd{
    ...
    % 
    Figure~\ref{fig:shouldnt}.right displays the irreversible constrained spectra associated to both evolutions, representing the uniqueness and stability of the corresponding solution trajectories.
    ...
    Indeed, the L-BFGS/CG method shifts the homogeneous solution to a distinct trajectory that diverges from the homogeneous path even before the bifurcation load, despite the stability of the homogeneous branch,  exhibiting a spurious bifurcation not associated with a stability transition.    % Indeed, despite the stability spectrum undergoes an abrupt change at the bifurcation load, the state is therein stable.
    ...
    At higher loads on the contrary, the initial condition is a nontrivial perturbation mode close to the optimal constrined mode, leading to a more accurate estimation of the stability threshold.
    % 
    In Fig.~\ref{fig:irreversibility-profiles}.left (resp., right) we plot the damage state $\alpha$ and corresponding bifurcation eigenmode $p$ (corresponding to point A in Figure~\ref{fig:shouldnt}.right) and stability eigenmodes $p^+$ (corresponding to point B in \ref{fig:shouldnt}.right) at the bifurcation loading step $\bar \epsilon_b$, namely point $H$ in the energy diagram Fig.~\ref{fig:branches-stiff}.
