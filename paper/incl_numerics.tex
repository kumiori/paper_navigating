...
\label{eq:stifness2}
\end{equation}
Our objective is to establish a branch switching strategy as part of our model's dynamic expansion. This extension must ensure the system's re-stabilization following an instability in a dissipative manner. Furthermore, in a quasi-static scenario, it should involve the selection of a new locally stable equilibrium branch with inherently lower energy.  Considering applications in structural mechanics, our approach to selecting the new equilibrium branch should involve utilizing a criterion of local energy minimization (LEM)\comment[id=ALB]{Acronyms, imho, make it harder to read by requiring extra mental buffer.}  which emulates the zero viscosity limit of overdamped viscous dynamics. This approach disregards the global energy minimizing (GEM) strategy, which may be relevant in biomechanical applications~\cite{Salman2021-mn}. According to LEM protocol, during quasi-static loading, the system will remain in a metastable state (a local minimum of energy) until it becomes unstable. Subsequently, during an isolated switching event, the new equilibrium branch will be chosen using a descent algorithm \cite{Puglisi2005-lg}.
\begin{figure}
    %\centering
...
$$
\Psi(y)-\Psi(y_t)\geq \frac{h^2}{2}\lambda_0 \|w_0\|^2, \quad \forall y: \|y - y_t\| \leq \delta,
% \delta = O(\| y-y_t\|),
% +o(h^2\|w_n\|^2).
$$
\added[id=ALB]{where $\lambda_0$ indicates the smallest eigenvalue and $w_0$ the associated eigenmode. The first order term $\delta\Psi(y_t)(v_n, \alpha_n)$ vanishes identically owing to the fact that bifurcation modes are admissible fields for the equilibrium condition. 
...
the convergence of quasi-Newton algorithms depends on the construction of an (approximate) Hessian which is strictly positivity.
As a consequence, such approximation systematically shields the ability to capture the sign change of the smallest eigenvalue of the Hessian. 
...
\label{eqn:energy-slice}$$
 to find an optimal value \(\eta\), such that $\eta \in \arg \min_{\eta} f$. Instead, we \deleted[id=ALB]{initially}  set \(\eta = 1\)  (this choice is further discussed in the following) \added[id=ALB]{and run the quasi-Newton step to obtain a new critical point $\mathbf{\tilde X}$.}
...
\replaced[id=ALB]{Our numerical findings suggest that equilibiria with small but positive eigenvalues of the Hessian are robust in the sense that they are insensitive to perturbations with the corresponding eigenvectors. This is highlighed by the fact that the quasi-Newton solver yields, after convergence, the same equilibrium state prior to perturbation.
On the other hand, when the smallest eigenvalue of the system's Hessian is small and negative,
small values of \( \eta \) obtained by minimisation of \eqref{eqn:energy-slice} is not sufficient for the  quasi-Newton algorithm to effectively escape the flat region of the energy landscape. 
...
To escape this region, one can design an iterative algorithm that repeatedly performs line searches in the current unstable direction followed by minimization, iteratively flowing along the escape path. However, for the current problem, it was sufficient to take the step-size \( \eta=1 \) to escape the unstable state that solves the first-order problem \deleted[id=ALB]{\( \delta \Psi=0 \)}, even though this choice us suboptimal \replaced[id=ALB]{with respect to}{increases the function} \( f(\eta) \). 
...
As a pointwise constraint, irreversibility restricts branch switching events in such a way that, e.g., the transition between branches with $n=3$ to $n=4$  is not allowed (cf. profiles in the Figure~\ref{fig:kick1} of the transition (b)$\mapsto$ (d)) because it requires a local decrease of the damag field. 
On the flip side, irreversibility acts as a global constraint on the perturbation space rendering the analysis more involed because of the strong nonlinearity induced by the constraint.
}
...
More precisely, we plot the film displacement at equilibrium $u_t$ which is linear and coincides with the substrate's displacement, and the homogeneous damage field $\alpha_t$. Furthermore, we represent the profile of the damage eigenfunction associated to the minimal eigenvalue both for the second order bifurcation~\eqref{eq:variational_inequality} (continuous black line) and stability problem~\eqref{eq:variational_stability}.
The bifurcation profiles coincide with the eigenmodes $n=4, 5$ alredy found, cf.~\eqref{} and are computed at two loads $t_b < t_1 < t_2$.
}
\added[id=ALB]{
...
Note that the bifurcation spectrum is singular at $t=0$. This follows the fact that, in this model, the damage criterion is attained as soon as $t=0$, thus the space of perturbations changes suddenly from $\emptyset$ at $t=0$ to the vector space $V_0$ for $t>0$. Conversely, the set of admissible perturbations for the stability problem changes from the empty at $t=0$ set to the solid cone $\{\beta\in H^1((0,1)):\beta \geq 0\}$ only beyond the bifurcation load, that is for $t>t_b>0$.
In the inset, we zoom on the infimum of the spectra which shows the transition from the uniqueness of the homogeneous branch to the existence, for $t \geq t_b$, of a multiplicity of equilibrium curves passing through the current homogeneous state, each of which associated to a bifurcation mode.
