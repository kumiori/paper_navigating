...
% $\mathbf{H}^i, i = 1, 2$
$\stiffmat$ and $\widetilde\stiffmat$, \albadd{which are real, symmetric, block matrices of dimensions ${n_u + n_\alpha}$ and ${2n_u + n_\alpha}$, respectively}. The explicit form of the stiffness matrix for the stiff substrate model is given by
\begin{equation}
    \stiffmat =
    % \stiffmat^1=
    \begin{bmatrix}
...
\albadd{
    \begin{equation}
        ...
        \begin{bmatrix}
            ...
            \right] dx \\
            \int_0^1 \left[
                (\frac{1}{2} u'^2 a'(\alpha) + w'(\alpha)) \mathcal{N}_i + \ell^2 \alpha' \mathcal{N}_i'
                \right] dx
