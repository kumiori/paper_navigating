...
\end{equation}
where $\epsilon(x)=u'(x)$ the continuum strain variable and $u(x)$ is the corresponding displacement field, \added[id=ALB]{$\lambda_1, \lambda_2$\comment[id=ALB]{Notation: $\ell, \ell_1$} are two nondimensional lengthscales, the former identifying the peak material stress as well as the typical scale of damage localisation bands and the latter quantifying the film's stiffness relative to the substrate.
}. 
...
Qualitative properties of AT2 model: damage starts as soon as $t>0$}
...
We also choose the boundary conditions in the form: $u(0)=-\bar\epsilon/2$, $u(1)=\bar\epsilon/2$\comment[id=ALB]{`natural' bcs moved down} and we impose the displacement field  of the substrate   as $u^s(x)=\bar \epsilon/2 (2x-1)$.
...
By using standard arguments of the calculus of variations, we localise the integral relation above to establish the strong (local) form of equilibrium conditions, and proceed with the evaluation of the equilibrium configuration via the Euler-Lagrange equations
\begin{eqnarray}
    % \label{modeld_el_1}
\begin{cases}
...
\end{eqnarray}
\added[id=ALB]{
    The differential system above is equipped with kinematic (essential) boundary conditions $u(0) = u^s(0), u(1) = u^s(1)$ as well as the natural conditions on the internal field $\alpha'(0)=\alpha'(1)=0$.
...
Notice that the choice of boundary conditions for displacements compatible with the substrate's deformation implies that $u(x)\equiv u^s(x)$\comment[id=ALB]{notation $
\bar u_t$} is (always) a solution to the elastic equilibrium equations with given homogeneous damage state. This makes it immediate to identify one fundamental (homogeneous) solution $y_h$ and decouple the elasticity problem along homogeneous evolutions from the damage-driven stress softening.
}
Therefore, solving~\eqref{auto1} for states such that $u''(x)=\alpha'(x)= 0, \, \forall x\in (0, 1)$ allows to determine the homogeneous solution $y_h = (u_h, \alpha_h)$ which reads \deleted[id=ALB]{solution to~\eqref{auto1} is becomes for the damage  field}, 
\begin{equation}
u_h(x) =  \bar \epsilon/2 (2x-1),\qquad\alpha_h = \frac{\Bar{\epsilon}^2}{2 + \Bar\epsilon^2}\label{eq:homo1},
\end{equation}\comment[id=ALB]{notation $\bar \epsilon_t$}
...
% 
By substituting the homogeneous solution $\alpha_h$ into \eqref{hessian1}, we compute  the $\det \mathcal{H}$, as depicted  in Fig. \ref{fig:hessian1}. The calculations are performed for the parameter values \replaced[id=ALB]{$\lambda_1=0.16$}{$\lambda_1=0.158114$} and $\lambda_2=0.34$.
\added[id=ALB]{Interpretation of the image: for a given load $\bar \epsilon$, $n(\bar \epsilon)$ where $n\in \mathbb N$ gives the wavenumber of associated to energy-decreasing damage perturbations. For a given loading history of increasing loads $\bar \epsilon(t)$, the wavenumber increases.}
...
\added[id=ALB]{
The last two relations are, respectively, the weak form of the mechanical equilibrium conditions and the threshold law for the evolution of the damage field.
The former allows to compute the elastic equilibrium displacment $u_t$ which is thus eliminated from energy\comment[id=ALB]{miracle in 1d, expand}, while the latter is a threshold law for the evolution of the damage field.
The variational inequality satisfied by the damage field in~\eqref{eq:eq_variational_inequality} takes a particularly expressive form when written as a complementarity problem, helping highlighting the mechanical nature of the damage criterion as a threshold law.
Namely,~\eqref{eq:variational_equilibrium} can be reduced to seeking $\alpha_t$ such that 
}
\added[id=ALB]{
    \begin{equation}
...
\end{equation}
where $\phi_t$ is the scalar function associated to the density of the energy variation defined by $\delta \Psi(y_t)(0, \beta - \alpha_t) = \langle -\phi_t(\alpha_t), \beta - \alpha_t\rangle$.
...
the energy density variation at equilibrium by the sound structure, and all equilibrium solutions $y_t$ such that $-\phi_t(\alpha_t) > 0$ belong to the interior of the damage yield surface.
The equality $-\phi_t(\alpha_t) = 0$, conversely, identifies the attainment of the state $y_t$ of the damage criterion as a yield surface, and the complementarity condition $\phi_t(\alpha_t)\dot \alpha_t = 0$ ensures that the damage field evolves only when the energy release rate is critical.
...
\end{equation}\comment[id=ALB]{for notational convenience $h\mapsto w$} 
The attainment of the equality in the last condition corresponds to the attainment of the damage yield surface or, equivalently, the criticality of the energy release rate. This, in turn, implies  the evolution of the damage field.
...
$$
The latter is an algebraic equation for $\alpha_t$ which is satisfied by the evolution of the  homogeneous damage response, as a function of the given loading level $t$.
The investigation of the stability properties of equilibria in the irreversible case requires  
considering second order energy variations with respect to all admissible perturbations that render null the first order term.
In general, this entails distinguishing between the regions where the damage criterion is attained and the evolution of the damage field is allowed from the complementary domain, where the second inequality in~\eqref{eq:complementarity} is satisfied with a strict inequality, and thus damage does not evolve.
In the current one-dimensional setup, the existence of nontrivial homogeneous solutions simplifies the analysis in that, as soon as the damage criterion is attained, damage can increase throughout the whole domain, thus the function space of admissible damage perturbations is defined on the fixed domain $(0, 1)$.
} 
...
As $t$ varies, $y_t$ describes a (smooth) curve in the phase space identified by its tangent vector $\dot y_t$ and its initial condition.
A fundamental question is to discern whether $y_t$ is an isolated equilibrium tracing a unique evolution path, or conversely it is the intersection of multiple equilibrium curves.
...
To this end, we can derive~\eqref{eq:eq_variational_inequality_full} with respect to $t$ to obtain a boundary value problem relating the rate of evolution $\dot y_t$ with the current state $y_t$, supposing the latter known, namely
% 
\begin{equation}
...
By construction, the homogeneous rate $\dot y_h$ is a solution, the question is whether another solution exists. The uniqueness is ensured by the positive definiteness of the quadratic form on the homogeneous linear space generated by $K^+_0$, namely $H^1_0((0,1)) \times H^1((0,1))$. 
...
\begin{equation}
    ...
    \delta^2 \Psi(y_t)(\dot\zeta, \dot\zeta) > 0, \quad \forall \dot\zeta \in V_0 = H^1_0((0,1)) \times H^1((0,1))    
\end{equation}
...
The first load at which the bifurcatio inequality~\eqref{eq:bifurcation_uniqueness} fails, namely $t_b:=\inf_t \{\delta^2 \Psi(y_t)(\zeta, \zeta) =0, \forall \zeta \in V_0 \}$ corresponds to the first critical bifurcation load, for which exist (multiple) equilibrium curves through $y_{t_b}$ and thus the possibility of bifurcating away from homogeneous branch. Notice that the possibility of bifurcation is, however, not an energetic necessity.
}

\added[id=ALB]{
Let's now consider the question of the stability of the homogeneous solution in the irreversible case. According to our energetic viewpoint, the stability of a state $y_t$ is governed by the positivity of $\delta^2 \Psi(y_t)$ on the constrained space of admissible state perturbations $K^+_0$.
The construction of the bifurcation problem is functional to infer a partial response on the stability, indeed if the current equilibrium branch is unique then the current state is necessarily stable (the converse is not true).
% , yet the converse is not true.
By construction, the set inclusion $K^+_0 \subseteq V_0$ implies that, denoting $t_s$ the load at which the homogeneous soltution looses stability by analogy to the bifurcation load, namely $t_s: = \inf_t \{\delta^2 \Psi(y_t)(\zeta, \zeta) =0, \forall \zeta \in K^+_0 \}$, necessarily $t_b \leq t_s$.
}
\added[id=ALB]{
...
\end{equation}
where $r_{\rm s}$\comment[id=ALB]{check notation} is the ratio between elastic constants of substrate and thin film and $u_s$ is the displacement field on the deformable non-breakable substrate with $\epsilon_{\rm s}=u_s'$ represents the deformation field on the elastic foundation. We  choose the boundary conditions in the form: $u(0)=-\bar\epsilon/2$, $u(1)=\bar\epsilon/2$,  $u_s(0)=-\bar\epsilon/2$, $u_s(1)=\bar\epsilon/2$ and, $\alpha'(0)=\alpha'(1)=0$.  The Euler-Lagrange equations read
...
\label{auto2}
\end{eqnarray}
It is easy to show that the homogeneous solution on the trivial branch remains the same as in the case of the rigid substrate, such that $\alpha_h = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2}\label{eq:homo11}$, whereas the effective elastic energy along the trivial branch now reads $f^0(\bar{\epsilon}, \alpha_h(\bar{\epsilon})) = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2} + \frac{r_s}{2}\bar{\epsilon}^2$.\comment[id=ALB]{check notation} 

\subsection{Linear stability}
We once more seek the incremental stability of an equilibrium configuration $u$, $\alpha$, and $u_s$, such that the first variation $\delta \Psi(u,\alpha,u_s)\added[id=ALB]{(y-y_t)}=0$ \added[id=ALB]{for all ...} with
\begin{equation*}
...
We again proceed to extract information on the onset of instability expanding in Fourier series the fields $v$, $w$ and $z$ appearing in \eqref{hessian222} such that $v(x)=\sum_{n=1}^{\infty} a_{n} \sin \left(n \pi x+\phi_{n}\right), \quad w(x)=\sum_{n=1}^{\infty} b_{n} \cos \left(n \pi x+\psi_{n}\right),\quad z(x)=\sum_{n=1}^{\infty} c_{n} \sin \left(n \pi x+\theta_{n}\right)$\comment[id=ALB]{check notation}. Exactly as in the previous section, we claim that the system is stable iff $\delta^2 \Psi(u,\alpha,u_s)(v,w,z)>0$. The stability condition  takes the form:
