...
\added[id=ALB]{Denoting by $H^1((0, 1))$ the  (Sobolev) space of scalar functions defined on the unit interval which are square integrable with square integrable first (weak) derivatives, the energy $\Psi$ makes sense for pairs $(u, \alpha)$ laying in the Cartesian vector product space $V:=H^1((0,1))\times  H^1((0,1))$.}
...
We also choose the boundary conditions in the form: $u(0)=-\bar\epsilon_t/2$, $u(1)=\bar\epsilon_t/2$\comment[id=ALB]{`natural' bcs moved down} and we impose the displacement field  of the substrate as \added[id=ALB]{a loading mode parametrised by $t$,} $u^s(x, t)=\bar \epsilon_t/2 (2x-1)$.
...
intuitive idea that a state is stable (hence observable) if it is a local minimum of the energy  among admissible perturbations. In practice, given an initial condition $y_0$ at $t=0$ we seek a state $y_t:=(u, \alpha)_t$ that, for a given value of the control parameter $t$, satisfies time-dependent kinematic boundary conditions on the displacement variable and is locally energy minimal. For definiteness, we seek $y_t\in X_t = \{v\in H^1((0, 1)):v=u^s(x, t),\ x\in \{0, 1\}\}\times H^1((0,1))$} such that
\begin{equation}
    \label{eq:variational_global_ineq}
    \Psi(y_t) \leq \Psi(y_t + z),\quad \forall \text{ admissible } z.
\end{equation}
% 
In the expression above the admissibility of perturbations accounts for kinematic boundary conditions as well as possible additional constraints. 
% Define $X_t = H^1((0, 1))$
...
$$
\added[id=ALB]{and is valid for any $y$ that verifies kinematic boundary conditions, its damage component $\alpha$ being either unconstrained or such that $\alpha\geq\alpha_t$, if irreversibility is taken into account.}
% 
...
\end{eqnarray}
\added[id=ALB]{
    The differential system above is equipped with kinematic (essential) boundary conditions $u(0) = u^s(0, t), u(1) = u^s(1, t)$ and natural conditions on the internal field $\alpha'(0)=\alpha'(1)=0$.
...
\bar u_t$} and $\alpha_t(x) = \alpha_h$ a load-dependent constant to identify, is always a solution to the elastic equilibrium equations. This makes it immediate to identify the fundamental (homogeneous) solution branch $y_h$ which allows to decouple the elasticity problem from the  evolution of damage. 
%  driving stress softening.
}
Therefore, solving~\eqref{auto1} for states such that $u''(x)=\alpha'(x)= 0, \, \forall x\in (0, 1)$ identifies the homogeneous solution $y_h = (u_h, \alpha_h)$ which reads \deleted[id=ALB]{solution to~\eqref{auto1} is becomes for the damage  field}, 
\begin{equation}
...
and the effective \replaced[id=ALB]{total}{elastic} energy along the \replaced[id=ALB]{homogeneous}{trivial} branch  
\begin{equation}
    \label{eq:energy_homogeneous}
    f_h(\bar\epsilon) :=\Psi(\alpha_h, u_h) = \frac{\Bar{\epsilon}^2}{2 + \Bar\epsilon^2}
\end{equation}
...
We distinguish two scenarios for the study of stability of equilibria: i) that in which all small perturbations are admissible and belong to a (linear) vector space, and ii) the case where damage is subject to an irreversibility constraint, which results in a (nonlinear) system of inequalities. Tje first scenario corresponds to the case in which damage is free to evolve and is reversible, so that negative variations of damage are allowed and indeed may occur - if convenient from an energetic viewpoint. The second setting, instead, considers damage as an irreversible process by enforcing an irreducible one-directional pointwise growth constraint.}


\subsection{Linear stability}
\comment[id=ALB]{How to understand linear/nonlinear}
An equilibrium configuration $y_t:=(u,\alpha)_t$, is a state such that the first variation $\delta \Psi(u,\alpha)\added[id=ALB]{(v, w)}$ vanishes for all admissible test fields \added[id=ALB]{in $V_0$} \comment[id=ALB]{for admissible $(v, w)$'s}. 
...
The figure represents, for a given load $\bar \epsilon$, the wavenumber $n(\bar \epsilon)\in \mathbb N$ of possible energy-decreasing damage perturbations. For an increasing loading history $\bar \epsilon(t)\nearrow$, the wavenumber increases, indicating the tendency to nucleate a network of cracks with higher spatial frequency.}
...
Because the current state can only be compared to those of with equal or higher damage, the space of admissible perturbations becomes the set $K^+_0:=H^1_0((0, 1))\times \{w \in H^1((0, 1)): w(x) \geq 0 \text{ a.e. }x\in (0, 1)\}$.
By construction, such space is contained in the vector space of admissible unconstrained perturbations, namely $K^+_0\subseteq V_0$. Indeed, if $(v, \beta)\in K^+_0$, then $-(v, \beta) \in V_0$ but $-(v, \beta)\notin K^+_0$. This restriction has a direct impact on the variational characterisation of local minima and bears consequences on both for the first order (equilibrium) conditions and the second order (stability) problem.}
...
\end{equation}
% 
which has to hold for all test states $(v, w)$ such that $v-u^s \in H^1_0((0, 1))$ and $w\in H^1_0((0,1))$ provided $w \geq \alpha_t$, that is $(v-u^s, w - \alpha_t) \in K^+_{0}$. 
...
Upon elimination of the kinematic field the variational inequality takes a particularly expressive form when written as a complementarity problem. This highlights the mechanical nature of the damage criterion as a threshold law.
Namely, using~\eqref{eq:variational_equilibrium} we seek an irreversible map $t\mapsto\alpha_t$ such that 
}
\added[id=ALB]{
    \begin{equation}
...
\end{equation}
where $\phi_t$ is a scalar function associated to the density of the equilibrium energy variation and is defined by $\delta \Psi(y_t)(0, \beta - \alpha) = \langle -\phi_t(\alpha), \beta - \alpha\rangle$, where the subscript $t$ indicates that it is computed at equilibrium.
...
the density of the energy variation at equilibrium for the undamaged structure, and all equilibrium solutions $y_t$ such that $-\phi_t(0) > 0$ belong to the interior of the damage yield surface for the sound structure. The equality $-\phi_t(0) = 0$, conversely, identifies the attainment of the damage criterion for the sound stricture, or equivalently, that the state $y_t$ has reached the elastic limit surface. The inequality $-\phi_t(\alpha_t) \geq 0$ states that the elastic energy release is either smaller than or equal to the (marginal) cost of damage, whereas complementarity condition $\phi_t(\alpha_t)\dot \alpha_t = 0$ ensures that the damage field evolves only if the energy release rate is critical.
...
\added[id=ALB]{
In the current one-dimensional setup with homogeneous initial conditions $y_0=(0, 0)$ and compatible kinematic boundary conditions, the existence of a homogeneous solution implies that the damage criterion is attained everywhere throughout the bar, which greatly simplifies the analysis of the energetic properties of the system. 
...
The equality in the last condition corresponds to the attainment a critical energy release rate whereby the elastic energy released (the negative quadratic term) equals and feeds the damage evolution.
...
In general, this requires distinguishing between the regions where the damage criterion is attained, and thus damage can evolve, from the (complementary) domain where the second relation in~\eqref{eq:complementarity} is satisfied with a strict inequality, and thus damage cannot evolve.
...
By construction, the homogeneous rate $\dot y_h$ is a solution of \eqref{eq:variational_bifurcation}, the question is whether another solution exists. The uniqueness is ensured by the positive definiteness of the quadratic form on the homogeneous linear space generated by $K^+_0$, namely $V_0$. 
...
\added[id=ALB]{
formally coincides with the classical linear stability problem of the reversible case~\eqref{eqn:linear_second_order_stability}, yet has a different mechanical interpretation in terms of evolution rates and uniqueness of equilibrium branches.
...
Denoting $t_s$ the load at which the homogeneous sulution looses stability by analogy to the bifurcation load, namely $t_s: = \inf_t \{\delta^2 \Psi(y_t)(\zeta, \zeta) =0, \forall \zeta \in K^+_0 \}$, the set inclusion $K^+_0 \subseteq V_0$ implies that necessarily $t_b \leq t_s$.
}
\added[id=ALB]{
...
\label{auto2}
\end{eqnarray}
It is easy to show that the homogeneous solution on the trivial branch remains the same as in the case of the rigid substrate, such that $\alpha_h = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2}\label{eq:homo11}$, whereas the effective elastic energy along the trivial branch now reads $\tilde f_h(\bar{\epsilon}) = \frac{\bar{\epsilon}^2}{2 + \bar{\epsilon}^2} + \frac{r_s}{2}\bar{\epsilon}^2$.\comment[id=ALB]{check notation} 
