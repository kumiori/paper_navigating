...
This allows identyfing sequences of equilibrium configurations as critical states, to ascertain their stability, and describing the system's transition trajectories between different equilibrium states.
...
The first order variation of the energy functional $\Estiff$ in the direction $z:=(v, w)$ is given by the following linear form  
\begin{equation}
...
The associated discrete problem can be numerically solved by exploiting the orthogonality between the set $\conespace$ and its dual $K^*:=\{y \in H^1(0,1): \langle x,y \rangle \leq 0, \forall x \in \conespace\}$, cf.~\cite{moreau, pinto-da-costa}.
