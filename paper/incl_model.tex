...
\albadd{
    % 
    The choice of the quadratic damage potential brings the main conceptual difficulty of the evolution problem at the first loading step.
    The other commonly used model AT1 has, instead, an elastic phase during which variations of damage are suboptimal. As a consequence, second or conditions (bifurcation and stability) are trivial, because damage cannot evolve. With our choice, starting from the smallest discrete load, damage can evolve and the space of variations of damage is nontrivial. A \emph{solid} cone, in the irreversible case.
    In this sense, the choice of the model is such that the first numerical challenge occurs at the first time step, marking
    the conceptual difference between the irreversible stability and bifurcation problems when irreversibility is accounted for.
    %  becomes manifest.
    Simplifying the numerical implementation,
    the lower and upper lower bounds (0 and 1) on the damage variable do not require
    additional constraints for first order solvers because it is satisfied by minimality.
    This allows to focus on the implementation of time-irreversibility at the level of second order solvers,
    exploiting the ability to explore a complex bifurcation landscape with continuation methods.
}
% as with the date in the following the numerical the solution of the second order condition isnontrivial
% , instead of having to wait for the elastic face and solve a long series of linear elastic problems     
% 
% Economic standpoint, we only solve one trivia zero load initial Evolution step
...
% 
The displacement field which is elastically compatible to an homogeneous strain in the substrate is the linear function $v(x, t) = \bar\epsilon_t/2 (2x-1)$, where \albadd{$\bar\epsilon_t\in \mathbb R$} is the \albdelete{applied tensile}\albadd{average} strain and $t$ plays the role of a loading time parameter.
% 
The film is subjected to \albadd{the displacement of the substrate} $v(x, t)$ \albdelete{as a tensile imposed load} and to given (compatible) displacements at its free ends $x = \{0, L\}$, so that for all $t$, $u(0)=v(0, t)$ and $u(L)=v(L, t)$
% 
...
\albadd{The reduced elastic model in~\eqref{def:energy_stiff} (and~\eqref{def:energy_compliant}) can be obtained as the asymptotic vanishing thickness limit for a thin 3d elastic bilayer system of thickness  $\mathsf 2h \ll L$, as shown in~\cite{Leon_Baldelli2015-rp}. Assuming the film is attached to its substrate by the means of an interface layer of stiffness $\Youngsubs \ll \Youngfilm$, the nondimensional constants appearing in the expression of the reduced-dimension energies can be related to the three-dimensional parameters of a phyisical mechanical system.}

\albdelete{Nondimensional constants appearing in the expression of the energies can be related \albadd{to} the three-dimensional parameters of a phyisical mechanical system \albdelete{ by an appropriate de-scaling}, remarking that the \albreplace{underlying}{reduced} elastic model \albadd{in}~\eqref{def:energy_stiff} \albadd{(a membrane supported by a linear elastic foundation)} can be obtained as \albreplace{the}{an} asymptotic \albadd{vanishing thickness} limit \albreplace{for}{starting from} a thin \albreplace{bi}{multi}layer system in 3d elasticity, as shown in~\cite{Leon_Baldelli2015-rp}\albadd{, upon appropriate scaling assumptions on the thickness and stiffness ratios}.}
\albdelete{In essence, a bilayer system  }
% , \mathsf{E'_\text{2d}}
...
\begin{equation}
    ...
    \Lambda := \frac{\Youngfilm}{\Youngsubs}\albadd{\frac{\mathsf h^2}{L^2}}, \quad
...
\end{equation}\comment[id=ALB]{unsure on last parameter}
\albadd{
    Note that the nondimensional parameter $\Lambda$ relates the effective stiffness of the film membrane undergoing planar tensile strains (i.e. $\Youngfilm h$), to the effective stiffness of the bonding layer which, to accommodate the mismatch deformation between the film and the undelying rigid substrate, is subject to a simple shear for which the effective stiffness is inversely proportional to its thickness, namely $\Youngsubs / h'$.}
\albdelete{while spatial variables and physical displacements are respectively normalized with respect to the film's length $L$ and the displacement scale $u_0 := \frac{\mathsf{w_1}L}{\mathsf{E_{\text{2d}} h}}$.}
...
we seek \albadd{stable states exploiting a notio of directional stability.}
...
\albadd{
    ...
    For small perturbations, this is a practical criterion that can be tested numerically without requiring a global search which is computationally prohibitive in infinite-dimensional spaces due to the vastness of the solution landscape.
    ...
    The directional minimality statement above is appealing because it avoids the pitfalls of both global and local minimality. Unlike local minimality, it leverages the topology of the real line, whis is intrinsic. This aligns with physical intuition that systems tend to evolve along a \emph{path} according to stability and constraints.
}
% This is a unilateral statement, incorporating the interplay between irreversibility and local minimisation, clarifying the concept of \emph{equilibrium} in a way that is consistent with the numerical approach.
...
Admissible perturbations in the fully reversible case they belong to the \albadd{tangent} space \albadd{$T_y X_t=X_0$} associated with  $X_t$ (respectively, $\widetilde{X_t}$) \albadd{which is a linear vector space}, whereas in the irreversible case \albreplace{admissible perturbations constitute}{they are bound to} the closed \albdelete{, pointed, }convex cone $K^+_0$ (respectively, $\widetilde{K^+_0}$).
...
\begin{equation}
    ...
    \left[\soften(\alpha)u'w'+\frac{1}{\elastell^2} (u-\subsu) w+ \left( \frac{1}{2}u'^2 \soften'(\alpha)+\homogdiss'(\alpha) \right)\beta+\damagell^2\alpha'\beta' \right]dx,\label{firstvar1}
\end{equation}\comment[]{REV: what about when $\alpha=1$?}
where
...
By using standard arguments of the calculus of variations, localizing the integral and choosing $\beta = 0$ first, and then $v =0$ leads to establishing the strong form of local equilibrium conditions which couple the mechanical equilibrium and the damage criterion, \albadd{and the associated boundary conditions, } respectively given by
\begin{eqnarray}
    % \label{modeld_el_1}
    \begin{cases}
...
\albdelete{This differential system is equipped with essential kinematic boundary conditions $u(0) = \subsu(0, t), u(1) = \subsu(1, t)$. Conversely, conditions on the internal field $\alpha'(0)=\alpha'(1)=0$ naturally follow by minimality.}
% When the order parameter is reversible, the space of admissible perturbations is the homogeneous space associated with  $X_t$, that is $V_0:= H^1_0((0,1))\times  H^1((0,1))$.
...
\albadd{For $x \in (0, 1)$} the Euler-Lagrange equations and associated boundary conditions read
\begin{eqnarray}
    \label{modeld_el_1}
    \begin{cases}
...
\begin{equation}
    \delta^2 \Ecompl(y_t)(y-y_t, y-y_t)=\int_0^1 \left[(1-\alpha_t)^2v'^2
        %+ \elastell^{-2}v^2
...
Similar to the previous section we claim that, whenever the first order term vanishes, the system is stable only if $\delta^2 \Ecompl
    ...
    >0$ for all sufficiently smooth admissible test fields $(v, \beta, \subsutest)$ in the vector space $\widetilde{ X_0}$. The stability condition \albadd{for the homogeneous state $\yhom$}  takes the form
\begin{align}
...
\end{equation}
Here, $\phi_t$ is the scalar function associated with  the variation of elastic energy density \albadd{computed} at the equilibrium \albadd{and} defined by $\delta \Estiff(y_t)(0, \beta) = \langle -\phi_t(\alpha_t), \beta\rangle$. \albdelete{Here, the subscript $t$ indicates that this quantity is computed {for} the equilibrium $u_t$}.
